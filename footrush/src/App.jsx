...
function BrandLogo({ compact = false }) {
  return (
    <div className="flex items-center gap-3">
      <div className={`overflow-hidden rounded-2xl border border-emerald-400/30 bg-white/10 ${compact ? "h-12 w-12" : "h-14 w-14"}`}>
        <img
          src={logoSrc}
          alt="FOOTRUSH logo"
      ...
      <div>
        <div className={`font-black tracking-tight text-emerald-400 ${compact ? "text-2xl" : "text-3xl"}`}>FOOTRUSH</div>
        <div className="text-xs font-medium uppercase tracking-[0.35em] text-emerald-100/70">Footwear Store</div>
      </div>
    </div>
  );
}
...
function ProductCard({ product, onAddToCart, onToggleWishlist, wishlist, onOpen }) {
  ...
  return (
    <div className="overflow-hidden rounded-[28px] border border-white/10 bg-white/5 shadow-[0_20px_60px_rgba(0,0,0,0.25)] backdrop-blur-md transition hover:-translate-y-1 hover:border-emerald-400/30">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="max-h-[90vh] w-full max-w-4xl overflow-auto rounded-[32px] border border-white/10 bg-[#0c1327] p-6 text-white shadow-2xl">
        ...
        <button
          onClick={onClose}
          className="mt-6 rounded-2xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-semibold text-white hover:bg-white/10"
        >
          Close
        </button>
      </div>
...
function LoginPage({ onCustomerLogin, onAdminLogin }) {
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#050813] text-white">
      ...
      <div className="absolute -left-20 top-16 h-72 w-72 rounded-full bg-emerald-500/20 blur-3xl" />
      ...
      <div className="relative z-10 mx-auto grid min-h-screen max-w-7xl items-center gap-12 px-6 py-12 lg:grid-cols-2 lg:px-8">
        <div>
          <BrandLogo />
          <div className="mt-8 inline-flex rounded-full border border-emerald-400/20 bg-emerald-500/10 px-4 py-2 text-sm font-medium text-emerald-300">
          ...
          <p className="mt-6 max-w-xl text-lg leading-8 text-white/75">
            A premium footwear shopping experience with customer login, wishlist, cart, orders, admin dashboard, product management, and a fully loaded modern interface.
          </p>
          <div className="mt-10 grid max-w-xl grid-cols-3 gap-4">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur">
              <Package className="mb-3 h-5 w-5 text-emerald-300" />
              <div className="text-2xl font-black">80+</div>
              <div className="text-sm text-white/60">Footwear Styles</div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur">
              <Truck className="mb-3 h-5 w-5 text-emerald-300" />
              <div className="text-2xl font-black">Fast</div>
              <div className="text-sm text-white/60">Delivery Support</div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur">
              <BadgeCheck className="mb-3 h-5 w-5 text-emerald-300" />
              <div className="text-2xl font-black">4.9★</div>
              <div className="text-sm text-white/60">Premium Rating</div>
            </div>
          </div>
        </div>
            ...
            <div className="grid gap-5">
              <div className="rounded-[28px] border border-white/10 bg-black/20 p-5">
                ...
                <div className="space-y-3">
                  <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/60">customer@footrush.com</div>
                  <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/60">••••••••</div>
                  <button
                    onClick={onCustomerLogin}
                    className="flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-5 py-4 font-semibold text-white hover:bg-emerald-400"
                  >
              ...
              <div className="rounded-[28px] border border-white/10 bg-black/20 p-5">
                ...
                <div className="space-y-3">
                  <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/60">admin@footrush.com</div>
                  <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/60">••••••••</div>
                  <button
                    onClick={onAdminLogin}
                    className="flex w-full items-center justify-center gap-2 rounded-2xl bg-cyan-500 px-5 py-4 font-semibold text-white hover:bg-cyan-400"
                  >
              ...
              <div className="rounded-[28px] border border-white/10 bg-white/5 p-5 text-sm text-white/65">
                ...
                Customer flow includes product browsing, wishlist, cart, orders, support, and admin access includes dashboard, products, orders, and client overview.
              </div>
            </div>
          </div>
        </div>
  ...
  const NavButton = ({ id, label }) => (
    <button
      ...
      className={`rounded-xl px-4 py-2 font-semibold transition ${
        page === id ? "bg-emerald-500 text-white shadow-lg shadow-emerald-500/30" : "bg-white/5 text-white hover:bg-white/10"
      }`}
    >
      {label}
    </button>
    ...
    <div className="min-h-screen bg-[#050813] text-white">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="hidden items-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-500 to-cyan-500 px-5 py-2 font-semibold text-white hover:opacity-90 md:flex"
            >
            ...
            <button
              onClick={logout}
              className="hidden items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-2 font-semibold text-white hover:bg-white/10 md:flex"
            >
        ...
        {menuOpen && (
          <div className="border-t border-white/10 bg-[#08101f] px-6 py-4 md:hidden">
            <div className="flex flex-col gap-2">
              ...
              <button
                onClick={logout}
                className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 font-semibold text-white"
              >
                Logout
              </button>
            </div>
      ...
      <main className="mx-auto max-w-7xl px-6 py-6">
        {role === "customer" && page === "home" && (
          <>
                ...
                <div>
                  ...
                  <div className="mt-8 flex flex-wrap gap-4">
                    <button
                      ...
                      className="rounded-2xl bg-emerald-500 px-6 py-4 font-semibold text-white shadow-lg shadow-emerald-500/30 transition hover:scale-105 hover:bg-emerald-400"
                    >
                      Shop Now
                    </button>
                  ...
                  <div className="mt-10 grid max-w-xl grid-cols-3 gap-4">
                    ...
                    <div className="rounded-3xl border border-white/15 bg-white/10 p-4 backdrop-blur">
                      <div className="text-2xl font-black">80+</div>
                      <div className="text-sm text-white/70">Shoe Styles</div>
                    </div>
                    <div className="rounded-3xl border border-white/15 bg-white/10 p-4 backdrop-blur">
                      <div className="text-2xl font-black">4.9★</div>
            ...
            <section className="mt-8 grid gap-4 md:grid-cols-4">
              <div className="rounded-[28px] border border-white/10 bg-gradient-to-br from-white/10 to-emerald-500/10 p-6 shadow-sm backdrop-blur">
              ...
              <div className="rounded-[28px] border border-white/10 bg-gradient-to-br from-white/10 to-cyan-500/10 p-6 shadow-sm backdrop-blur">
              ...
              <div className="rounded-[28px] border border-white/10 bg-gradient-to-br from-white/10 to-lime-500/10 p-6 shadow-sm backdrop-blur">
              ...
              <div className="rounded-[28px] border border-white/10 bg-gradient-to-br from-white/10 to-teal-500/10 p-6 shadow-sm backdrop-blur">
        ...
        {role === "customer" && page === "shop" && (
          <section className="space-y-6 rounded-[32px] border border-white/10 bg-white/5 p-8 shadow-sm backdrop-blur">
        ...
        {role === "customer" && page === "wishlist" && (
          <section className="rounded-[32px] border border-white/10 bg-white/5 p-8 shadow-sm backdrop-blur">
            ...
            <div className="mt-6 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {products.filter((product) => wishlist.includes(product.id)).length === 0 ? (
                <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white/65">No wishlist items yet.</div>
              ) : (
                products
          ...
          <div className="grid gap-8 lg:grid-cols-[1.4fr_0.8fr]">
            <section className="rounded-[32px] border border-white/10 bg-white/5 p-8 shadow-sm backdrop-blur">
              ...
              <div className="mt-6 space-y-4">
                {cart.length === 0 ? (
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white/65">Your cart is empty.</div>
                ) : (
                  cart.map((item) => (
                    <div key={item.id} className="flex flex-col gap-4 rounded-[24px] border border-white/10 bg-black/20 p-4 sm:flex-row sm:items-center">
                      ...
                      <div className="flex items-center gap-3">
                        <button onClick={() => updateCartQty(item.id, "dec")} className="rounded-full border border-white/10 bg-white/5 p-2"><Minus className="h-4 w-4" /></button>
                        <span className="font-semibold">{item.qty}</span>
                        <button onClick={() => updateCartQty(item.id, "inc")} className="rounded-full border border-white/10 bg-white/5 p-2"><Plus className="h-4 w-4" /></button>
                      </div>
                    </div>
                  ))
                )}
            ...
            <aside className="rounded-[32px] border border-white/10 bg-white/5 p-8 shadow-sm backdrop-blur">
              <h3 className="text-2xl font-black">Order Summary</h3>
              <div className="mt-6 space-y-3 text-sm">
                <div className="flex justify-between"><span className="text-white/60">Items</span><span>{cart.length}</span></div>
                <div className="flex justify-between"><span className="text-white/60">Total</span><span>₹{cartTotal}</span></div>
              </div>
              <button className="mt-6 w-full rounded-2xl bg-emerald-500 px-5 py-4 font-semibold text-white hover:bg-emerald-400">
                Proceed to Checkout
              </button>
            </aside>
          </div>
        ...
        {page === "contact" && (
          <section className="rounded-[32px] border border-white/10 bg-white/5 p-8 shadow-sm backdrop-blur">
            ...
            <div className="mt-6 space-y-4 text-white/80">
              <div className="flex items-center gap-3"><Phone className="h-4 w-4 text-emerald-300" /> +91 98765 43210</div>
              <div className="flex items-center gap-3"><Mail className="h-4 w-4 text-emerald-300" /> care@footrush.com</div>
              <div className="flex items-center gap-3"><MapPin className="h-4 w-4 text-emerald-300" /> Hyderabad, India</div>
            </div>
          </section>
        )}
        ...
        {role === "admin" && page === "admin" && (
          <div className="space-y-8">
            <section className="rounded-[32px] border border-white/10 bg-gradient-to-r from-zinc-950 via-slate-900 to-emerald-950 p-8 text-white shadow-sm">
              <div className="inline-flex rounded-full bg-white/10 px-4 py-2 text-sm text-emerald-300">FOOTRUSH Admin Panel</div>
              <h1 className="mt-4 text-4xl font-black">Store Dashboard</h1>
              <p className="mt-3 max-w-2xl text-white/65">Monitor clients, orders, inventory, and revenue.</p>
            </section>

            <section className="grid gap-4 md:grid-cols-3 xl:grid-cols-4">
              <div className="rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-sm backdrop-blur">
        ...
        {role === "admin" && page === "adminProducts" && (
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
            <section className="rounded-[32px] border border-white/10 bg-white/5 p-8 shadow-sm backdrop-blur">
              ...
              <div className="mt-6 space-y-4">
                <input value={adminProduct.name} onChange={(e) => setAdminProduct({ ...adminProduct, name: e.target.value })} placeholder="Product name" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none placeholder:text-white/35 focus:border-emerald-400" />
                <select value={adminProduct.category} onChange={(e) => setAdminProduct({ ...adminProduct, category: e.target.value })} className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none focus:border-emerald-400">
                  {categories.filter((c) => c !== "All").map((category) => <option key={category} className="bg-slate-900">{category}</option>)}
                </select>
                <div className="grid gap-4 sm:grid-cols-2">
                  <input value={adminProduct.price} onChange={(e) => setAdminProduct({ ...adminProduct, price: e.target.value })} placeholder="Price" className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none placeholder:text-white/35 focus:border-emerald-400" />
                  <input value={adminProduct.oldPrice} onChange={(e) => setAdminProduct({ ...adminProduct, oldPrice: e.target.value })} placeholder="Old Price" className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none placeholder:text-white/35 focus:border-emerald-400" />
                </div>
                <input value={adminProduct.stock} onChange={(e) => setAdminProduct({ ...adminProduct, stock: e.target.value })} placeholder="Stock" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none placeholder:text-white/35 focus:border-emerald-400" />
                <input value={adminProduct.image} onChange={(e) => setAdminProduct({ ...adminProduct, image: e.target.value })} placeholder="Image URL" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none placeholder:text-white/35 focus:border-emerald-400" />
                <button onClick={addAdminProduct} className="w-full rounded-2xl bg-emerald-500 px-6 py-4 font-semibold text-white hover:bg-emerald-400">
                  Add Product
                </button>
              </div>
            </section>
            ...
            <section className="rounded-[32px] border border-white/10 bg-white/5 p-8 shadow-sm backdrop-blur">
              ...
              <div className="mt-6 space-y-4 max-h-[640px] overflow-auto pr-1">
                {products.map((product) => (
                  <div key={product.id} className="flex items-center gap-4 rounded-[24px] border border-white/10 bg-black/20 p-4">
        ...
        {selectedProduct && (
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            <div>
              ...
              <div className="mt-6">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {selectedProduct.sizes.map((size) => (
                    <span key={size} className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white">
                      {size}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                <button onClick={() => addToCart(selectedProduct)} className="rounded-2xl bg-emerald-500 px-6 py-4 font-semibold text-white hover:bg-emerald-400">
                  Add to Cart
                </button>
                <button onClick={() => toggleWishlist(selectedProduct.id)} className="rounded-2xl border border-white/10 bg-white/5 px-6 py-4 font-semibold text-white hover:bg-white/10">
                  Wishlist
                </button>
              </div>
            </div>
